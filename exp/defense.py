...
if args.defender == "SafeDecoding":
    adapter_path = Path("..") / "lora_modules" / args.model_name
    if not (adapter_path.is_dir() and (adapter_path / "adapter_config.json").exists()):
        raise FileNotFoundError(f"Missing LoRA at: {adapter_path}")
    model = PeftModel.from_pretrained(model, str(adapter_path), adapter_name="expert")

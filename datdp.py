...
class LocalModelAdapter:
    """Adapter that wraps a local model to expose a GPT-API-like interface."""
    ...
    def __call__(self, prompt: str) -> str:
        """Simulate a GPT API call by generating a response with the local model."""
        # Create a temporary copy of the conversation template
        import copy
        ...
        gen_config.max_new_tokens = 512  # Reasonable length for the evaluation
            ...
            with torch.no_grad():
                outputs = self.model.generate(
                    **inputs,
                    ...
                    do_sample=True,  # Add randomness to avoid repetitive responses
                    temperature=0.7,
                    top_p=0.9
                )
...
> {submitted_prompt}"""
    ...
    def run_evaluation_call(self, system_prompt: str, user_prompt: str) -> str:
        ...
        # Merge the system prompt and the user prompt
    ...
    def parse_verdict(self, raw_text: str) -> Tuple[str, str]:
        ...
        if yes_matches and no_matches:
            # Pick the one that appears later in the text
            ...
            # Otherwise scan the trailing tokens
            words = text_lower.split()
            if len(words) >= 1:
                for word in reversed(words[-5:]):  # Check the last 5 words
